...
class CategoriaDao():
  ...
  def get_categorias_by_gerente(self, gerente):
    categorias = None
    try:  
      categorias = Categoria.objects.raw("SELECT c.categoria_id, c.categoria_nombre FROM categoria c INNER JOIN rbs r ON c.rbs_id = r.rbs_id WHERE r.gerente_id = %s", [gerente.gerente_id])
    except Error as e:
      print(e)
    finally:      
      return categorias
