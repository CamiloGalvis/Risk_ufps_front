...
class ProyectoHasRiesgo_ActividadDao():
	...
	def registrar_actividad_riesgo(self, proyecto_riesgo, actividad):
		with closing(connection.cursor()) as cursor:
			cursor.execute(
                'INSERT INTO riesgos_bd.`proyecto_has_riesgo_actividad`(`proyecto_has_riesgo_id`, `actividad_id`)'
                'VALUES (%s, %s)',
	...
	def listar_actividades_riesgo(self, proyecto_id):
		actividades = {}
		try:
			actividades = Actividad.objects.raw("SELECT * FROM actividad act INNER JOIN proyecto_has_riesgo_actividad pr ON act.actividad_id=pr.actividad_id INNER JOIN proyecto_has_riesgo tr ON pr.proyecto_has_riesgo_id=tr.proyecto_has_riesgo_id WHERE tr.proyecto_id = %s", [proyecto_id])
