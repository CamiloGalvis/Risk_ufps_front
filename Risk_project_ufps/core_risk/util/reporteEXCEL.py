...
class reporteEXCEL(object):
    ...
    def __init__(self, titulo, cabecera, registros, nombreEXCEL, propietario):
    ...
    def Exportar(self):
      ...
      # ===================== INFORMACIÓN EXTRA ====================
        ...
        celdaOrigen.value = "Generado por: "+ self.propietario
      ...
      # ============== AJUSTAR ANCHO (CELDAS - TABLA) ==============
        ...
        for col in hoja.columns:
            ...
            if columna:
                ...
                hoja.column_dimensions[get_column_letter(columna[0][0])].width = ajustarAncho
    ...
    def Exportar_planificar(self, objetivo, alcance):
      ...
      # ========= DATOS - BORDES - COLOR (CABECERA - TABLA) ========
        mi_row = mi_row + 2
        for indice, dato in enumerate(self.cabecera, start=2):
            hoja.cell(row=mi_row, column=indice).value = dato
            hoja.cell(row=mi_row, column=indice).border = border
            hoja.cell(row=mi_row, column=indice).alignment = centrarTexto
            hoja.cell(row=mi_row, column=indice).font = Font(color="FF000000", size=10, bold=True)
      ...
      # ====== REGISTROS - BORDES - COLOR (REGISTROS - TABLA) ======
        mi_row = mi_row + 1
        for filaIndice, registros in enumerate(self.registros, start=mi_row):
            for columnaIndice, registro in enumerate(registros, start=2):
                hoja.cell(row=filaIndice, column=columnaIndice).value = registro
                hoja.cell(row=filaIndice, column=columnaIndice).border = border
...
#def generarReporte():
    ...
    cabecera = ("D.N.I", "NOMBRE", "APELLIDO", "FECHA DE NACIMIENTO", "super 2", "vuela")
